                ...
                <form action="{{ route('invoices.update', $invoice->id) }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <select name="user_id" id="user_id" class="form-select select2" required>
                            ...
                            @foreach ($users as $id => $name)
                                <option value="{{ $id }}"
                                    {{ (old('user_id') ?? $invoice->user_id) == $id ? 'selected' : '' }}>{{ $name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="amount" id="amount" class="form-control currency-input"
                            value="{{ old('amount') ?? number_format($invoice->amount, 0, ',', '.') }}" required>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="is_paid" id="is_paid" class="form-select">
                            <option value="0" {{ (old('is_paid') ?? $invoice->is_paid) == 0 ? 'selected' : '' }}>Belum
                                Dibayar</option>
                            <option value="1" {{ (old('is_paid') ?? $invoice->is_paid) == 1 ? 'selected' : '' }}>Sudah
                                Dibayar</option>
                        </select>
                    </div>
